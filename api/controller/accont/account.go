...
func Route(engine *gin.Engine) {
	save := func(c *gin.Context) {
		...
		err := accountService.A.Login(a) // 校验shareId
		if err != nil {
			util.RenderJSON(c, http.StatusBadRequest, err.Error())
			return
		}
		a.Token = uuid.New().String()
		a.Expire = time.Now().Add(time.Hour * time.Duration(24))
		err = accountService.A.Save(a) // 保存会话
		if err != nil {
			util.RenderJSON(c, http.StatusInternalServerError, err.Error())
			return
		}
		c.SetCookie("token", a.Token, (int)((time.Hour*time.Duration(24))/time.Second), "/", "*", false, false)
